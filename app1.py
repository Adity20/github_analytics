...
def create_mindsdb_project_and_train(username, repo_name):
    ...
    project = server.get_project('github_analytics') # Replace 'state' with actual target column
    ...
    if repo_name:
        ...
        if repo_data:
            ...
            if issues:
                ...
                st.write(f'Open Issues: {open_issues}, Closed Issues: {closed_issues}')
                st.write('Labels:', Counter(labels))

                # Identify tech stack
                tech_stack = identify_tech_stack([issue['title'] + " " + issue.get('body', '') for issue in issues])
                st.write('Tech Stack Demanded:')
                st.write(tech_stack)

                # Predict issue priorities
                issue_descriptions = [issue['title'] + " " + issue.get('body', '') for issue in issues]
                predictions = predict_issue_priority(issue_descriptions)
                st.write('Predicted Issue Priorities:')
