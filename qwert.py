...
if username:
    if repo_name:
        ...
        if repo_data:
            ...
            if issues:
                ...
                st.write(f"Labels: {Counter(labels)}")

                issue_descriptions = [issue['title'] + ". " + issue.get('body', '') for issue in issues]

                st.subheader('Issue Solutions')
                issue_solutions = provide_recommendations(issues)
                solutions_df = pd.DataFrame([{
                    'Issue': issue_descriptions[i],
                    'Solution': issue_solutions[i]
    ...
    st.write("Please enter a GitHub username in the sidebar to get started.")
